...
chrome.runtime.onInstalled.addListener(async (details) => {
  try {
    ...
    const keysToSave = Object.keys(configs).filter(
      (k) =>
        storedConfigs[k] === undefined || k === "isFirefox" || k === "isMac",
    );
    if (keysToSave.length > 0) {
      ...
      if (
        keysToSave.includes("dbclickToPreview") &&
    ...
    .then((currentWindow) => {
      return loadUserConfigs()
        .then((userConfigs) => {
          ...
          const filteredPopupWindowsInfo = Object.keys(popupWindowsInfo).reduce(
            (acc, key) => {
        ...
        .then(({ popupWindowsInfo, userConfigs }) => {
          ...
          if (request.action === "windowRegainedFocus") {
            const popupWindowsInfo = userConfigs.popupWindowsInfo || {};
            const isCurrentWindowOriginal = popupWindowsInfo.hasOwnProperty(
              currentWindow.id,
            );
            ...
            if (isCurrentWindowOriginal) {
              ...
              Object.keys(popupWindowsInfo[currentWindow.id] || {}).forEach(
                (popupId) => {
          ...
          if (request.action === "sendPageBack") {
            ...
            if (popupWindowsInfo && Object.keys(popupWindowsInfo).length > 0) {
              ...
              if (originalWindowId) {
                const createData = {
                  windowId: parseInt(originalWindowId),
                  url: sender.tab.url,
                };
                if (
              ...
              console.error(
                "popupWindowsInfo is empty or not properly structured.",
              );
            }
            ...
            getZoomFactor()
              ...
              .catch((error) => {
                sendResponse({
                  error:
                    error.message ||
                    "Error occurred while retrieving zoom factor.",
                });
              });
          ...
          return getZoomFactor()
            .then((zoom) => {
              ...
              if (
                request.lastClientX !== undefined &&
                request.lastClientY !== undefined
              ) {
...
function createPopupWindow(
  ...
  chrome.storage.local.get(
    ...
    (result) => {
      const enableContainerIdentify =
        result.enableContainerIdentify !== undefined
          ? result.enableContainerIdentify
          : true;
      ...
      if (
        result.rememberPopupSizeAndPositionForDomain &&
        popupWindowsInfo.savedPositionAndSize
      ) {
        if (popupWindowsInfo.savedPositionAndSize[domain]) {
        ...
        chrome.windows.create(
          {
            ...
            type: windowType,
            top: parseInt(
              savedPositionAndSize ? savedPositionAndSize.top : top,
            ),
            left: parseInt(
              savedPositionAndSize ? savedPositionAndSize.left : left,
            ),
            width: parseInt(
              savedPositionAndSize ? savedPositionAndSize.width : width,
            ),
            height: parseInt(
              savedPositionAndSize ? savedPositionAndSize.height : height,
            ),
            focused: true,
            incognito:
              tab && tab.incognito !== undefined ? tab.incognito : false,
            ...(enableContainerIdentify &&
              tab.cookieStoreId &&
          ...
          (newWindow) => {
            if (chrome.runtime.lastError) {
              ...
              // extend for mac
              if (result.isMac) {
                if (!openPopups.includes(tabs[0].id))
...
function defaultPopupCreation(
  ...
  dx = Math.max(
    lastScreenLeft,
    Math.min(dx, lastScreenLeft + lastScreenWidth - defaultWidth),
  );
  dy = Math.max(
    lastScreenTop,
    Math.min(dy, lastScreenTop + lastScreenHeight - defaultHeight),
  );
...
function updatePopupInfoAndListeners(
  linkUrl,
  newWindow,
  originWindowId,
  popupWindowsInfo,
